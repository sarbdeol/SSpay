...
const MENUS = {
  SUPER_ADMIN: [
    ...
    },
    {
      ...
      icon: HiOutlineCash,
      path: "/superadmin/expense-entries",
    },
  ],
  ADMIN: [
    ...
    {
      label: "Expense Managers",
      icon: HiOutlineDocumentText,
      path: "/admin/expense-managers",
    },
  ...
  SUB_MERCHANT: [
    ...
    // { label: 'Daily Report', icon: HiOutlineDocumentReport, path: '/submerchant/daily-report' },
    {
      label: "Ledger",
      icon: HiOutlineDocumentText,
      path: "/submerchant/ledger",
    },
    {
      label: "Configurations",
      icon: HiOutlineCog,
      children: [{ label: "Transactions", path: "/submerchant/transactions" }],
    },
  ],
  AGENT: [
    ...
    { label: 'Settlement Details', icon: HiOutlineCog, path: '/agent/settlement-details' }, // ← ADD
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-3 h-9 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-mac"
      >
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-green-600">
                    {parseFloat(rates.aedTodayRate).toFixed(4)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">USDT Rate</span>
                  <span className="font-semibold text-blue-600">
                    {parseFloat(rates.usdtTodayRate).toFixed(4)}
                  </span>
                </div>
              </div>
            ) : (
...
function SidebarItem({ item, collapsed }) {
  ...
  if (item.children) {
    return (
      <div>
        <button
          onClick={() => setOpen(!open)}
          className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-xl transition-mac"
        >
          ...
          <div className="ml-8 mt-1 space-y-0.5">
            {item.children.map((c) => (
              <NavLink
                ...
                to={c.path}
                className={({ isActive }) =>
                  `block px-3 py-2 text-sm rounded-lg transition-mac ${isActive ? "text-brand-500 bg-brand-50 font-medium" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`
                }
              >
                {c.label}
              </NavLink>
  ...
  return (
    <NavLink
      to={item.path}
      className={({ isActive }) =>
        `flex items-center gap-3 px-4 py-2.5 text-sm rounded-xl transition-mac ${isActive ? "bg-brand-500 text-white shadow-sm" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`
      }
    >
      <Icon className="w-5 h-5 flex-shrink-0" />
...
export default function DashboardLayout({ children }) {
  const { user, logout, stopImpersonating, impersonationStack, jumpToStack } =
    useAuth();
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-40 bg-white border-r border-gray-100 flex flex-col transition-all duration-300 ${sidebarOpen ? "w-64" : "w-20"} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          {menuItems.map((item, i) => (
            <SidebarItem key={i} item={item} collapsed={!sidebarOpen} />
          ))}
        </nav>
        <div className="p-3 border-t border-gray-100">
          {impersonationStack.length > 0 && (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-amber-600 hover:bg-amber-50 rounded-xl transition-mac mb-1"
            >
              <HiOutlineSwitchHorizontal className="w-5 h-5" />
              {sidebarOpen && (
                <span>
                  Back to{" "}
                  {impersonationStack[impersonationStack.length - 1].user.name}
                </span>
              )}
            </button>
          )}
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 rounded-xl transition-mac"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-6">
          <button
            ...
            className="p-2 rounded-lg hover:bg-gray-100 transition-mac"
          >
        ...
        {impersonationStack.length > 0 && (
          <div className="border-b border-gray-100 px-4 py-2 bg-white shadow-sm">
  <div className="flex items-center gap-1.5 text-sm flex-wrap">
    {/* Impersonation icon */}
    <span className="text-xs bg-amber-100 text-amber-700 font-semibold px-2 py-0.5 rounded-full mr-1">
      👤 Viewing As
    </span>
    ...
    {impersonationStack.map((entry, i) => (
      <React.Fragment key={i}>
        <button
          onClick={() => {
            jumpToStack(i);
            navigate(ROLE_ROUTES[entry.user.role] || "/");
          }}
          className="flex items-center gap-1 px-2 py-0.5 rounded-md text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-all text-xs font-medium"
        >
          {entry.user.name}
    ...
    <div className="flex items-center gap-1.5 px-2.5 py-0.5 bg-brand-50 border border-brand-200 rounded-md">
      <span className="w-1.5 h-1.5 rounded-full bg-brand-500 animate-pulse"></span>
      <span className="font-semibold text-brand-700 text-xs">{user?.name}</span>
      <span className="text-brand-400 text-xs">·</span>
      <span className="text-brand-500 text-xs">{user?.role?.replace(/_/g, " ")}</span>
    </div>
  </div>
</div>
        )}
