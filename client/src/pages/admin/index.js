...
function EditUserDetailModal({ open, onClose, entity, entityType, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const entityPayload = { name: form.name, description: form.description, isActive: form.isActive };
      if (entityType === "merchant") {
        entityPayload.maxPaymentLimit = form.maxPaymentLimit;
        entityPayload.commissionChargePercent = form.commissionChargePercent;
      }
      if (entityType === "agent") {
        entityPayload.commissionChargePercent = form.agentCommissionChargePercent;
      }
      ...
      const userPayload = {};
      if (form.username && form.username !== entity.user?.username) userPayload.username = form.username;
    ...
    <Modal open={open} onClose={onClose} title={title} maxWidth="max-w-xl">
      {entity && (
        <form onSubmit={handleSubmit}>
          {/* ── Profile Info ── */}
          <div className="mb-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">Profile Info</p>
            <FormInput
              label="Name"
              required
              value={form.name}
            ...
            {entityType === "merchant" && (
              <>
                <FormInput
                  ...
                  onChange={(e) => setForm({ ...form, maxPaymentLimit: e.target.value })}
                ...
                <FormInput
                  ...
                  onChange={(e) => setForm({ ...form, commissionChargePercent: e.target.value })}
                  placeholder="e.g. 2.5"
                />
              </>
            )}
          ...
          <div className="border-t border-gray-100 pt-3 mt-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">Login Credentials</p>
            <FormInput
              label="Username"
              value={form.username}
...
export function AdminMerchants() {
  ...
  return (
    <div>
      <PageHeader
        title="Merchants"
        action={<Button onClick={() => setShowCreate(true)}>Add Merchant</Button>}
      />
      ...
      <DataTable
        ...
        actions={(row) => (
          <div className="flex gap-1">
            <button
              onClick={() => { navigator.clipboard.writeText(`Username: ${row.user?.username}\nPassword: ${row.user?.plainPassword || 'N/A'}\nLogin: ${window.location.origin}/login`); toast.success('Copied!'); }}
              className="p-1.5 rounded-lg hover:bg-gray-100 text-sm" title="Copy credentials"
            >📋</button>
            <button
              onClick={() => setEditEntity(row)}
              className="p-1.5 rounded-lg hover:bg-yellow-50 text-yellow-500" title="Edit details"
            >✏️</button>
            <button
              onClick={async () => { try { await impersonate(row.user?.id); navigate('/merchant'); } catch(e) { toast.error('Failed.'); } }}
              className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500" title="Login as merchant"
            >🔑</button>
            <button
              onClick={() => handleToggle(row)}
              className={`p-1.5 rounded-lg transition-mac ${row.isActive ? "hover:bg-red-50 text-red-400" : "hover:bg-green-50 text-green-500"}`}
            >{row.isActive ? "⏸" : "▶️"}</button>
          </div>
        )}
      />
      ...
      <DataTable
        ...
        actions={(row) => (
          <div className="flex gap-1">
            ...
            >✏️</button>
            <button
              onClick={async () => { try { await impersonate(row.user?.id); navigate('/agent'); } catch(e) { toast.error('Failed.'); } }}
              className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500" title="Login as agent"
            >🔑</button>
          </div>
        )}
      />
...
export function AdminCollectors() {
  ...
  return (
    <div>
      <PageHeader
        title="Collectors"
        action={<Button onClick={() => setShowCreate(true)}>Add Collector</Button>}
      />
      <DataTable
        columns={columns}
        data={collectors}
        ...
        actions={(row) => (
          <div className="flex gap-1">
            ...
            >✏️</button>
            <button
              onClick={async () => { try { await impersonate(row.user?.id); navigate('/collector'); } catch(e) { toast.error('Failed.'); } }}
              className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500" title="Login as collector"
            >🔑</button>
          </div>
        )}
      />
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        actions={(row) => (
          <button onClick={() => setShowDetail(row)} className="text-brand-500 text-sm hover:underline">
            View All
          </button>
        )}
      />
...
export function AdminConfiguration() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setForm({ type: "merchant", entityId: "", usdtTodayRate: "", aedTodayRate: "", currency: "AED" });
      fetchRates();
    } catch (e) {
  ...
  const handleEdit = async (e) => {
    e.preventDefault();
    try {
      const data = { usdtTodayRate: editRate.usdtTodayRate, aedTodayRate: editRate.aedTodayRate };
  ...
  const getMerchantName = (id) => merchants.find((m) => m.id === id)?.name || "-";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-card p-6">
          ...
          <form onSubmit={handleUpdate}>
            <FormSelect label="Type" options={[{ value: "merchant", label: "Merchant" }, { value: "agent", label: "Agent" }]} value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value, entityId: "" })} />
            <FormSelect label={form.type === "merchant" ? "Select Merchant" : "Select Agent"} required placeholder="Select..." options={(form.type === "merchant" ? merchants : agents).map((e) => ({ value: e.id, label: e.name }))} value={form.entityId} onChange={(e) => setForm({ ...form, entityId: e.target.value })} />
            <FormInput label="USDT Today Rate" required type="number" step="0.0001" value={form.usdtTodayRate} onChange={(e) => setForm({ ...form, usdtTodayRate: e.target.value })} placeholder="e.g. 92" />
            <FormInput label="AED Today Rate" required type="number" step="0.0001" value={form.aedTodayRate} onChange={(e) => setForm({ ...form, aedTodayRate: e.target.value })} placeholder="e.g. 26.05" />
                ...
                <div key={rate.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-mac">
                  <div>
                    <p className="text-sm font-semibold text-gray-800">
                      {rate.merchantId ? `Merchant: ${getMerchantName(rate.merchantId)}` : ""}
                      {rate.agentId ? `Agent: ${getAgentName(rate.agentId)}` : ""}
                  ...
                  <button onClick={() => setEditRate({ ...rate, aedTodayRate: parseFloat(rate.aedTodayRate), usdtTodayRate: parseFloat(rate.usdtTodayRate) })} className="p-1.5 rounded-lg hover:bg-gray-100 text-sm transition-mac">✏️</button>
                </div>
              ))}
            </div>
          )}
        ...
        {editRate && (
          <form onSubmit={handleEdit}>
            <p className="text-sm text-gray-500 mb-4">
              {editRate.merchantId ? `Merchant: ${getMerchantName(editRate.merchantId)}` : ""}
              {editRate.agentId ? `Agent: ${getAgentName(editRate.agentId)}` : ""}
            </p>
            <FormInput label="AED Today Rate" required type="number" step="0.0001" value={editRate.aedTodayRate} onChange={(e) => setEditRate({ ...editRate, aedTodayRate: e.target.value })} />
            <FormInput label="USDT Today Rate" required type="number" step="0.0001" value={editRate.usdtTodayRate} onChange={(e) => setEditRate({ ...editRate, usdtTodayRate: e.target.value })} />
    ...
    <div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Collection Form">
        <form onSubmit={handleCreate}>
          ...
          <FormSelect label="Select Merchant" required placeholder="Select Option" options={merchants.map((m) => ({ value: m.id, label: m.name }))} value={form.merchantId} onChange={(e) => setForm({ ...form, merchantId: e.target.value })} />
          <FormTextarea label="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Optional description" />
      ...
      <div className="bg-white rounded-2xl shadow-card p-6 max-w-3xl">
        <div className="flex gap-4 mb-6">
          <div>
            ...
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="h-10 px-3 border border-gray-200 rounded-xl text-sm" />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1 block">End Date</label>
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="h-10 px-3 border border-gray-200 rounded-xl text-sm" />
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        page={1}
        actions={(r) => (
          <button onClick={() => handleDelete(r.id)} className="text-red-500 text-sm hover:underline">Unblock</button>
        )}
      />
              ...
              Object.values(merchantGroups).map((group, gIdx) => {
                ...
                return remarkEntries.map(([remark, amount], rIdx) => (
                  <tr key={`${gIdx}-${rIdx}`} className={gIdx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    {rIdx === 0 && <td className="border border-gray-200 px-3 py-1.5 text-right font-medium align-top" rowSpan={remarkEntries.length}>₹{group.totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
                    {rIdx === 0 && <td className="border border-gray-200 px-3 py-1.5 text-right font-medium align-top" rowSpan={remarkEntries.length}>{todayAed.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
                    ...
                    {rIdx === 0 && <td className="border border-gray-200 px-3 py-1.5 text-right text-gray-400 align-top" rowSpan={remarkEntries.length}>-</td>}
                    {rIdx === 0 && <td className="border border-gray-200 px-3 py-1.5 text-right font-semibold text-red-600 align-top" rowSpan={remarkEntries.length}>{outstanding.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
                    {rIdx === 0 && <td className="border border-gray-200 px-3 py-1.5 text-right text-gray-400 align-top" rowSpan={remarkEntries.length}>-</td>}
                  </tr>
                ));
              })
