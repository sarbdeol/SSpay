      ...
      <DataTable columns={columns} data={admins} total={total} page={page} onPageChange={setPage} loading={loading}
        actions={(row) => (
          <div className="flex items-center gap-1">
            <button onClick={() => handleCopyCredentials(`Username: ${row.username}`)} title="Copy Username" className="p-1.5 rounded-lg hover:bg-gray-100 transition-mac text-sm">📋</button>
  ...
  return (
    <div>
      ...
      <DataTable columns={columns} data={users} total={total} page={page} onPageChange={setPage} loading={loading}
        ...
        actions={(row) => (
          <div className="flex items-center gap-1">
            <button onClick={() => navigator.clipboard.writeText(row.username).then(() => toast.success('Username copied!'))} title="Copy Username" className="p-1.5 rounded-lg hover:bg-gray-100 text-sm">📋</button>
